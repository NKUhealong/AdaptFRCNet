...
def train():
    ...
    def create_model(image_size, num_classes,ema):
        model = SegFormer_B4(image_size,num_classes)
        if ema:
    ...
    for name,p in model.named_parameters():
        if 'linear'  in name or 'pool' in name: 
    ...
    with open("./seg.txt","a") as f:
        txt = 'polyp seg_MT_dcoder_tuning'+'MAE:'+str(best_MAE)+' Acc:'+str(best_Acc)+' Dice:'+str(best_Dice)+' IoU:'+str(best_IoU)
        f.write(txt+'\n') 
train()